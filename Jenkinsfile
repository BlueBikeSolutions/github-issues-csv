node('master') {
    ...
    stage 'Building GH2CSV Docker Image'
    	sh 'docker build -t gh2csv:latest .'
    stage 'Running Issues Extractor Script'
		withCredentials([usernamePassword(credentialsId: 'ghsignin', passwordVariable: 'ghpass', usernameVariable: 'ghuser')]) {
	    	sh 'docker run --name gh2csv_script -v ${PWD}/issues:/opt/issues gh2csv python main.py -u {ghuser} -p {ghpass} -r {ghrepo} --no-quick >> /opt/issues/issues_{env.BUILD_NUMBER}.csv'
		}
}
